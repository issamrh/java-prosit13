...
public class Level3 {
    ...
    public static void main(String[] args) {
        ...
        String names = teachers.stream().map(Teacher::getName).map(String::toUpperCase).reduce((s1, s2) -> s1 + "#" + s2).orElse("");

        /* TO DO 2: Retourner une set d'enseignants Java dont le salaire > 80000 */
        Set<Teacher> teachers1 = teachers.stream().filter(e -> e.getSubject().equals("Java") && e.getSalary() > 80000).collect(Collectors.toSet());
       ...
       // Map<Integer, List<Teacher>> map = teachers.stream()./* TO DO 5 */;
        ...
        Map<Integer, String> map1 = teachers.stream().collect(Collectors.groupingBy(Teacher::getSalary, Collectors.mapping(Teacher::getName, Collectors.joining(","))));
